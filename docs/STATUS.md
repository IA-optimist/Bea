...
> Honest per-component maturity rating. Last verified: **2026-06-23**, merged to `main` from `fix/policy-engine-session-limits-singleton` + `fix/mission-id-propagation-audit`.
...
| Policy/tool guardrails cleanup | 🟢 Constantes `Decision` centralisées, `tool_executor` nettoyé, ratchet import interne activé, tracker de session partagé branché via `PolicyEngine.ensure_session()` + `check_limits()` + `record_action()`; `ToolExecutor`/`MetaOrchestrator`/`BeaOrchestrator` propagent et utilisent `get_policy_engine()` singleton (`fix/policy-engine-session-limits-singleton`) — **mission_id propagation audit terminé** (`fix/mission-id-propagation-audit`) : 3 call sites fixés, ratchet `scripts/check_tool_executor_mission_id.py`, 9 tests |

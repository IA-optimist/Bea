...
mission canonique. Son rerun exige également `ResourceGuard` en `NORMAL` ou
`SOFT_WARN` et un slot disponible. Lorsqu'il effectue un rerun, il retourne
uniquement le rapport dont le score historique est strictement supérieur. Il
n'écrit plus la tâche, le feedback ou le score dans `improvement_memory`. Il
n'est pas présenté comme une deuxième autorité du chemin standard.

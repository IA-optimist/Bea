...
|------|--------|------------------|-------------|
| `.env` | EXISTS | ⚠︝ YES (JWT_SECRET_KEY, POSTGRES_PASSWORD, OPENROUTER_API_KEY) | ❌ NO (in .gitignore) |
...
## 11. HARDENING JUNE 2026 — Auth / Principal Binding Chain

**Date:** 2026-06-24  
**Branches:** `feat/principal-auth-binding` + `fix/mission-submitted-by` + `fix/approval-queue-auth` + `beta/auth-session-hardening` (en cours)

### Ce qui a été corrigé

| Risque | Gravité | Statut |
|---|---|---|
| `principal_id` extrait de `params` (falsifiable client) | HIGH | ✅ FIXED — `get_authenticated_principal(request)` depuis `AccessEnforcementMiddleware` |
| `approved_by`/`rejected_by` depuis query params client | MEDIUM | ✅ FIXED — toujours depuis auth context |
| `approved_by="human"` hardcodé dans `approval_queue` | MEDIUM | ✅ FIXED — ratchet `check_approval_hardcoded_principals.py` |
| Sessions PolicyEngine indexées par `mission_id` seul | MEDIUM | ✅ FIXED — clé `principal_id:mission_id` |
| `submitted_by` non persisté → identité perdue à la reprise | HIGH | ✅ FIXED — stocké à la soumission, utilisé à la reprise |
| Approbateur utilisé comme identité d'exécution à la reprise | HIGH | ✅ FIXED — `_resume_principal = record.submitted_by or approver` |
| Session store in-memory non multi-worker | P3 | 🔄 EN COURS — `RedisSessionStore` sur `beta/auth-session-hardening` |

### Règle fondamentale établie

```
submitted_by  = identité d'exécution (PolicyEngine, session key)
approved_by   = audit uniquement (jamais passé à l'exécution)
rejected_by   = audit uniquement (jamais passé à l'exécution)
```

### Ratchets ajoutés

- `scripts/check_policy_principal_binding.py` — 24/24 call sites avec principal (0 gaps)
- `scripts/check_tool_executor_mission_id.py` — 6/6 call sites avec mission_id (0 gaps)
- `scripts/check_approval_hardcoded_principals.py` — 0 violations "human"/"admin"/"system"

### Secrets / logs

- `get_authenticated_principal()` retourne `access_token:<token_id>` ou `jwt:<sub>` ou `static:api` — jamais le token brut
- `PolicyEngine` n'accepte plus `_extract_principal_id(params)` sur le chemin `evaluate_tool()`
- Aucune API key, JWT ou bearer token n'est loggé ou utilisé comme clé de session

### Risques résiduels documentés

1. **P3 — Session store in-memory** : `PolicyEngine._sessions` reste en mémoire locale. Multi-worker non safe. `RedisSessionStore` en cours (`beta/auth-session-hardening`).
2. **P3 — LangGraph / autonomous runners** : certains chemins internes ne propagent pas `principal_id` à chaque tool call. Documenté dans `ALPHA_READINESS.md`.
3. **P3 — Legacy records** : anciens records sans `submitted_by` font fallback sur l'approbateur à la reprise. Comportement documenté, non bloquant.
...
- FIX 3 (CVSS 7.2 asyncpg pool) — ⚠︝ partiel → **appliqué** : `core/db/project_crud.py` bumped `min_size 2→5`, `max_size 10→20`, `max_queries=50k`, `max_inactive_lifetime=300s`, `command_timeout=60s`, `timeout=30s`
...
### 9. Principal / auth binding for PolicyEngine sessions (2026-06-24)


**Fix� dans `feat/principal-auth-binding`.**

- `api/auth_principal.py` fournit `get_authenticated_principal(request)` qui lit
  `request.state.user` positionn� par `AccessEnforcementMiddleware`. L'identit�
  retourn�e est stable et non secr�te (`<auth_type>:<id>`).
- Aucun token brut, cl� secr�te ni valeur `BEA_API_TOKEN` n'est utilis�e comme
  principal.
- Les routes publiques (`/api/v2/task`, `/api/v2/missions`, `/api/v1/missions`,
  `/api/v2/missions/{id}/approve`, `/api/v3/tools/{tool_id}/execute`,
  `/api/v2/tools/test`) injectent le principal valid� et �crasent toute valeur
  fournie par le client dans `params` (`principal_id`, `_bea_principal_id`).
- Le principal suit le chemin canonique jusqu'� `PolicyEngine.evaluate_tool()` :
  `KernelAdapter.submit()` -> `kernel.execute()` ->
  `MetaOrchestrator.run_mission()` / `BeaOrchestrator.run()` ->
  `tool_runner.run_tools_for_mission()` -> `execution_engine.execute_tool_intelligently()`
  -> `tool_pipeline_tool.tool_pipeline()` -> `_bea_principal_id` dans params.
- `PolicyEngine._extract_principal_id()` privil�gie `_bea_principal_id` (cl� de
  confiance) puis accepte `principal_id`/`user_id`/`tenant_id`/`owner_id` comme
  fallback pour appels internes/tests.
- `PolicyEngine.evaluate_tool()` re�oit `principal_id` en argument explicite et ne
  lit plus le principal dans `params`.
- Ratchet : `scripts/check_policy_principal_binding.py --summary` (24/24 clean).

**Dette restante :**
- Certains chemins d'ex�cution interne (LangGraph flow, autonomous runners,
  profilage) et les appels sans contexte de requ�te (dev/test) omettent le
  principal : retour � une cl� de session `mission_id` brute, acceptable en
  mono-tenant mais insuffisant pour une beta publique multi-tenant.
- `PolicyEngine._sessions` reste en m�moire process ; passage � l'�chelle
  multi-worker n�cessitera un store Redis partag�.

---

### 10. Mission submitter identity persistence (2026-06-24)

**Fix� dans `fix/mission-submitted-by`.**

- `core/mission_models.MissionResult`, `kernel.state.mission_state.MissionContext`
  et `core.mission_persistence.PersistedMission` portent d�sormais les champs
  `submitted_by` (principal du soumetteur) et `approved_by` (principal de
  l'approbateur, audit uniquement).
- Les endpoints publics de soumission (`/api/v2/task`, `/api/v2/missions/submit`,
  `/api/mission`, `POST /api/v1/missions`) extraient le principal authentifi� via
  `get_authenticated_principal(request)` et le stockent comme `submitted_by`.
  Aucune valeur fournie par le client (`submitted_by`, `_bea_principal_id`,
  `principal_id`) n'est accept�e. En mode production (`_REQUIRE_AUTH=true`),
  l'absence de principal d�clenche une erreur 401.
- � la reprise/approbation (`api.mission_approval.approve_mission_for_resume()` et
  `MetaOrchestrator.resolve_approval()`), l'ex�cution reprend avec le principal
  `submitted_by`, garantissant que la session `PolicyEngine` utilise la cl�
  `submitter:mission_id` et non `approver:mission_id`.
- `approved_by` n'est jamais utilis� comme identit� d'ex�cution ; il sert
  uniquement � l'audit.
- Les anciens enregistrements sans `submitted_by` restent lisibles et basculent
  en mode d�grad� sur le principal de l'approbateur lors de la reprise.
- Ratchets : `scripts/check_policy_principal_binding.py --summary` reste � 24/24.
  Nouveaux tests : `tests/test_mission_submitted_by.py`.

**Dette restante :**
- La persistance `submitted_by`/`approved_by` est en m�moire process / SQLite /
  JSON locale. Une mise � l'�chelle multi-worker n�cessitera de propager ces
  champs dans le store Redis partag� pr�vu pour `PolicyEngine._sessions`.

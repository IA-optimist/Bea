...
| `api/routes/missions.py` | HTTP routes plus mission orchestration glue | Request schemas are now in `api/schemas_missions.py`; agent output extraction is now in `api/mission_outputs.py`; mission response assembly is now in `api/mission_response.py`; agent list/trigger helpers are now in `api/mission_agents.py`; next split should move task submission orchestration. |

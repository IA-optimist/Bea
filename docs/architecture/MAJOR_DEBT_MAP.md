...
| `core/orchestration/execution_mixin.py` | Mission execution policy and supervised execution phases extracted from `MetaOrchestrator` | Split `_execute_supervised` into private execution policy/context helpers while preserving the mixin API. |

"""
HexStrike Process Router — Flask Blueprint for /api/process endpoints.

Extrait depuis hexstrike_server.py. Monté via app.register_blueprint(process_bp).
"""
...
from enhanced_process import EnhancedProcessManager
...
@process_bp.route("/execute-async", methods=["POST"])
...
@process_bp.route("/get-task-result/<task_id>", methods=["GET"])
...
@process_bp.route("/pool-stats", methods=["GET"])
...
@process_bp.route("/cache-stats", methods=["GET"])
def get_cache_stats():
...
@process_bp.route("/clear-cache", methods=["POST"])
...
@process_bp.route("/resource-usage", methods=["GET"])
...
@process_bp.route("/performance-dashboard", methods=["GET"])
...
@process_bp.route("/terminate-gracefully/<int:pid>", methods=["POST"])
...
@process_bp.route("/auto-scaling", methods=["POST"])
...
@process_bp.route("/scale-pool", methods=["POST"])
...
@process_bp.route("/health-check", methods=["GET"])
...
__all__ = ["process_bp", "enhanced_process_manager"]

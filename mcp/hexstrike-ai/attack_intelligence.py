"""
HexStrike Attack Intelligence — target modeling + decision engine.

Extrait depuis hexstrike_server.py pour isoler le sous-système de
modélisation d'attaque et le moteur de décision intelligent.

Classes (dans l'ordre des dépendances) :
  - TargetType (Enum) : catégories de cibles (web, network, api, ...)
  - TechnologyStack (Enum) : stacks détectables
  - TargetProfile (dataclass) : profil enrichi d'une cible
  - AttackStep (dataclass) : une étape d'une chaîne d'attaque
  - AttackChain (dataclass) : séquence d'AttackStep
  - IntelligentDecisionEngine : orchestration décisionnelle (986 l)

Backward-compat :
    from .attack_intelligence import (TargetType, TechnologyStack,
        TargetProfile, AttackStep, AttackChain, IntelligentDecisionEngine)
"""

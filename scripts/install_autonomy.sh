...
#
# What it does :
#   1. Verifies the API token is reachable in /root/Jarvismax-master/.env
#   2. Creates /etc/jarvismax/ with autonomy_objective.txt + autonomy.env
#   3. Copies deploy/jarvis-autonomy.service → /etc/systemd/system/
#   4. systemctl daemon-reload + enable --now jarvis-autonomy
#   5. Smoke test : curl /api/v3/autonomy/status
#
# Safety :
#   - Asks confirmation before each destructive step
#   - Backs up any existing service file
#   - Refuses to install if JARVIS_AUTONOMY_USE_REAL=1 isn't set explicitly
...
SVC_SRC="${REPO_DIR}/deploy/jarvis-autonomy.service"
SVC_DST="/etc/systemd/system/jarvis-autonomy.service"
CFG_DIR="/etc/jarvismax"
...
blue "════════════════════════════════════════════════════════════"
blue "  JarvisMax autonomy daemon — install"
blue "════════════════════════════════════════════════════════════"

# ── 1. Extract API token ─────────────────────────────────────
TOKEN=$(grep -E '^JARVIS_API_TOKEN=' "$ENV_FILE" | head -1 | cut -d= -f2-)
if [[ -z "$TOKEN" ]]; then
  die "JARVIS_API_TOKEN missing in $ENV_FILE"
fi
green "  ✓ token loaded from $ENV_FILE"

# ── 2. /etc/jarvismax/ config ────────────────────────────────
mkdir -p "$CFG_DIR"
chmod 750 "$CFG_DIR"
...
if [[ ! -f "$CFG_ENV" ]]; then
  cat > "$CFG_ENV" <<EOF
# /etc/jarvismax/autonomy.env — overrides for jarvis-autonomy.service
# Edit then : systemctl daemon-reload && systemctl restart jarvis-autonomy
...
confirm "  Enable + start jarvis-autonomy now ?"
systemctl enable jarvis-autonomy
systemctl start jarvis-autonomy
sleep 2
systemctl status --no-pager jarvis-autonomy | head -20
...
echo "  Restart bootstrap    : systemctl restart jarvis-autonomy"
echo "  Disable              : systemctl disable --now jarvis-autonomy"
echo "  Pause running daemon : echo JARVIS_AUTONOMY_PAUSED=1 >> .env && docker restart jarvis_core"
echo
yellow "  REMINDER : the daemon currently runs in SAFE mode unless the API"
yellow "  process has JARVIS_AUTONOMY_USE_REAL=1 in its environment. Add"

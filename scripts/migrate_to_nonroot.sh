#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────
# migrate_to_nonroot.sh — Migrate JarvisMax container to non-root USER
# ──────────────────────────────────────────────────────────────────
# Usage (sur VPS1, root) :
...
#   3. chown ~/.jarvismax vers UID:GID 1000
#   4. Swap Dockerfile → Dockerfile.nonroot
#   5. Adapte docker-compose.yml : volume mount → /home/jarvis/.jarvismax
#   6. Rebuild image
#   7. Restart container
...
NEW_DATA_DIR="${NEW_DATA_DIR:-/home/jarvis/.jarvismax}"
CONTAINER="${CONTAINER:-jarvis_core}"
HEALTH_URL="${HEALTH_URL:-http://localhost:8000/api/v2/health}"
TS="$(date +%Y%m%d-%H%M%S)"
BACKUP_DIR="${BACKUP_DIR:-/root/jarvismax-nonroot-migration.$TS}"
...
blue "════════════════════════════════════════════════════════════"
blue "  JarvisMax → Non-root USER migration — $TS"
blue "════════════════════════════════════════════════════════════"
echo
echo "Repo         : $REPO_DIR"
...
if id -u jarvis >/dev/null 2>&1; then
  EXISTING_UID=$(id -u jarvis)
  [[ "$EXISTING_UID" == "1000" ]] || die "user jarvis exists with UID=$EXISTING_UID (expected 1000)"
  green "  ✓ user jarvis (UID=1000) already exists"
else
  useradd --uid 1000 --user-group --create-home --shell /bin/bash jarvis
  green "  ✓ user jarvis created (UID=1000)"
fi
...
if grep -q "~/.jarvismax:/root/.jarvismax" "$COMPOSE"; then
  sed -i 's|~/.jarvismax:/root/.jarvismax|~/.jarvismax:/home/jarvis/.jarvismax|g' "$COMPOSE"
  green "  ✓ volume mount updated"
elif grep -q "/home/jarvis/.jarvismax" "$COMPOSE"; then
  green "  ✓ volume mount already points to /home/jarvis/.jarvismax"
else

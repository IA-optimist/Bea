...
Security:
  ...
  - Branch name is scoped to "bea/si-<run_id>"
...
GIT_USER_NAME = os.getenv("GIT_USER_NAME", "BeaMax-SI")
GIT_USER_EMAIL = os.getenv("GIT_USER_EMAIL", "bea-si@noreply.beamax.ai")
...
class GitAgent:
    ...
    def _create_worktree(self, snapshot: WorkspaceSnapshot, patch_id: str) -> WorkspaceSnapshot:
        ...
        tmp = _tf.mkdtemp(prefix=f"bea-wt-{patch_id}-")
        branch = f"bea/sandbox-{patch_id}-{int(time.time())}"
    ...
    def _create_tempcopy(self, snapshot: WorkspaceSnapshot) -> WorkspaceSnapshot:
        ...
        import shutil
        tmp = tempfile.mkdtemp(prefix="bea-si-")
    ...
    def _create_pr_flow(
        ...
        if has_changes:
            _run_git(["stash", "push", "-m", f"bea-si-stash-{run_id}"], cwd=root)
    ...
    def _do_branch_and_pr(
        ...
        commit_msg = (
            ...
            f"{explanation[:500]}\n\n"
            f"Generated by BeaMax Self-Improvement V3\n"
            f"Run ID: {run_id}\n"
    ...
    def _create_github_pr(
        ...
        try:
            ...
            title = f"[BeaMax SI] {domain}: self-improvement patch {run_id} (score={score:.2f})"
            body = (
                ...
                f"### Explanation\n{explanation[:1000]}\n\n"
                f"### Review Required\n"
                f"This PR was generated by BeaMax Self-Improvement V3.  \n"
                f"It was validated in a Docker sandbox (--network=none).  \n"
                f"**Manual review required before merging.**\n\n"
                f"---\n"
                f"*Generated by BeaMax SI | Auto-merge: DISABLED*"
            )

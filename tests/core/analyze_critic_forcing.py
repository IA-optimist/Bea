            ...
            with open(path, encoding="utf-8") as f:
                for line in f:
                    ...
                    try:
                        ...
                        continue  # ligne bruitée/incomplète : on ignore
        except OSError:
            continue
...
def summarize(name, deltas):
    ...
    print(f"  {name}: n={len(deltas)}  delta_moyen={statistics.mean(deltas):+.3f}"
...
def main():
    ...
    # Verdict pré-engagé sur les reruns FORCÉS (le groupe qu'on valide).
    print("\n# Verdict dette 1")
    if len(forced) < 5:
    ...
    if mean_f > 0 and pos_ratio > 0.5:
        ...
        print("  Le couplage viabilité→prudence améliore le travail marginal sous charge.")
    elif mean_f <= 0:

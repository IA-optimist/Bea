...
def main() -> None:
    ...
    out = []
    out.append("# chain_integrated_audit")
    out.append("## Attack 1 - session drift")
    out.append(f"first50_mean_abs={a1.first50_mean_abs:.6f} last50_mean_abs={a1.last50_mean_abs:.6f} slope={a1.slope:.8f} final={fmt(a1.final_state)} drift_ratio={a1.drift_ratio:.6f} verdict={a1.verdict}")
    out.append("## Attack 2 - stress desperation")
    out.append(f"min_valence={a2.min_valence:.6f} max_abs_guidance={a2.max_abs_guidance:.6f} extreme_turns={a2.extreme_turns} worst_guidance={a2.worst_guidance} verdict={a2.verdict}")
    out.append("## Attack 3 - boot and UNKNOWN")
    out.append(f"boot_state={fmt(a3.boot_state)} unknown_before={fmt(a3.unknown_before)} unknown_after={fmt(a3.unknown_after)} unknown_delta={a3.unknown_delta:.12f} verdict={a3.verdict}")
    out.append("## Attack 4 - noisy jitter")
    out.append(f"input_variance={a4.input_variance:.6f} output_variance={a4.output_variance:.6f} ratio={a4.ratio:.6f} verdict={a4.verdict}")
    sys.stdout.write("\n".join(out) + "\n")
